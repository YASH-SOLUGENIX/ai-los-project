...
This LOS backend uses **pgvector** for RAG/vector similarity search, so PostgreSQL must use a pgvector-enabled image.
...
```bash
docker exec -it los-postgres psql \
  -U los_user \
  -d los_db \
...
The LOS RAG implementation uses PostgreSQL `vector(768)` columns for embeddings generated by the `nomic-embed-text` Ollama model.
...
The LOS RAG tables use `vector(768)` embeddings.
...
- [ ] PostgreSQL container is running with pgvector
- [ ] PostgreSQL `vector` extension is enabled
